...
## اصول چندزبانه و متمرکزسازی پیام‌ها در ربات تلگرام بی‌واسط

### هدف
تمام پیام‌هایی که به کاربر نمایش داده می‌شود (راهنما، خطا، هشدار، دکمه و ...) باید فقط از طریق فایل `localization.py` و تابع `get_message` فراخوانی شوند تا:
- ترجمه و تغییر پیام‌ها ساده باشد.
- توسعه چندزبانه بدون تکرار و خطا انجام شود.
- تجربه کاربری یکپارچه و حرفه‌ای باشد.

### اصول و نکات
1. **عدم استفاده از متن مستقیم:**
   هیچ پیام متنی نباید به صورت مستقیم در کد نوشته شود. فقط کلید پیام در `localization.py` تعریف و با `get_message("key", lang=...)` فراخوانی شود.
2. **پشتیبانی از پارامترها:**
   اگر پیام نیاز به مقدار متغیر دارد (مثلاً نام دسته، مقدار عددی و ...)، از قالب‌بندی با kwargs استفاده شود.
3. **افزودن پیام جدید:**
   برای هر پیام جدید، ابتدا کلید مناسب در `localization.py` اضافه و سپس فقط کلید آن در کد استفاده شود.
4. **توسعه چندزبانه:**
   برای هر پیام، مقدار فارسی و انگلیسی (و زبان‌های دیگر در صورت نیاز) تعریف شود.
5. **بررسی و تست:**
   پس از هر توسعه، تست شود که پیام‌ها به درستی و با زبان مناسب نمایش داده می‌شوند.

### الگوی استفاده
```python
from localization import get_message
await message.reply_text(get_message("description_only_text", lang=context.user_data.get('lang', 'fa')))
```

### نتیجه
با رعایت این اصول، توسعه و نگهداری ربات بسیار ساده‌تر و حرفه‌ای‌تر خواهد بود و افزودن زبان جدید یا تغییر پیام‌ها بدون نیاز به ویرایش کد اصلی انجام می‌شود. این داکیومنت را به همه توسعه‌دهندگان پروژه ارائه دهید تا منطق چندزبانه و متمرکزسازی پیام‌ها در همه بخش‌ها رعایت شود.
