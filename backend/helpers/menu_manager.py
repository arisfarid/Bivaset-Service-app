...
class MenuManager:
    ...
    async def show_menu(update: Update, context: ContextTypes.DEFAULT_TYPE, text: str, 
        ...
        logger.debug(f"MenuManager.show_menu called, clear_previous={clear_previous}")
        
        # ابتدا سعی کن منوی موجود را edit کن
        if query:
            logger.debug("Attempting to edit existing menu via callback query")
            try:
                message = await query.message.edit_text(text, reply_markup=keyboard, parse_mode='Markdown')
                menu_id = message.message_id
                logger.debug(f"Successfully edited menu message ID {menu_id}")
                ...
                logger.debug(f"Could not edit menu via query: {e}")
                
                # اگر خطای "Message is not modified" است، یعنی محتوا تغییری نکرده
                if "Message is not modified" in str(e):
                    logger.debug("Menu content unchanged - returning current menu ID")
                ...
                # اگر edit نشد، ادامه به سناریوی ارسال پیام جدید        # اگر clear_previous درخواست شده، پیام‌های قبلی را حذف کن
        if clear_previous and 'menu_history' in context.user_data:
            logger.debug(f"Clearing {len(context.user_data['menu_history'][-5:])} previous menus")
            for msg_id in context.user_data['menu_history'][-5:]:
                try:
                    await context.bot.delete_message(chat_id=chat_id, message_id=msg_id)
                except (BadRequest, TelegramError) as e:
                    logger.debug(f"Could not delete menu message {msg_id}: {e}")
            # پاک کردن تاریخچه بعد از حذف
            context.user_data['menu_history'] = []

        # ارسال پیام جدید
        logger.debug("Sending new menu message")
        try:
            ...
            logger.debug(f"Sent new menu message with ID {menu_id}")
            return menu_id
